...
const QUICK = ["react", "express", "lodash", "axios", "typescript", "vite", "next", "tailwindcss"];

type NpmSearchResult = { name: string; version: string; description: string; keywords: string[] };
type NpmSearchResponse = {
  q: string; page: number; results: NpmSearchResult[];
...
export default function NpmSearchPage() {
  ...
  const { history, addQuery, removeQuery } = useSearchHistory("npm_search_history");

  const { data: cachedData } = useQuery({
    queryKey: ["npm", "cached"],
    queryFn: () => apiGet<{ packages: string[] }>("/api/npm/cached"),
    staleTime: 30_000,
  });
  const cachedSet = new Set(cachedData?.packages ?? []);
  ...
  const searchQ = useInfiniteQuery({
    ...
    initialPageParam: 1,
    queryFn: ({ pageParam = 1 }) =>
      apiGet<NpmSearchResponse>(`/api/npm/search?q=${encodeURIComponent(query)}&page=${pageParam}&limit=20`),
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "ArrowDown") { e.preventDefault(); setFocusedIdx((i) => Math.min(i + 1, results.length - 1)); }
      ...
      navigate(`/npm/package/${results[focusedIdx].name}`);
    ...
    <div>
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-semibold text-base tracking-tight">npm</span>
        ...
        <div className="relative">
          ...
          <input
            ...
            style={{ "--tw-ring-color": "hsl(var(--eco-npm)/0.35)" } as React.CSSProperties}
          />
      ...
      {!searchQ.isLoading && query && results.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <p className="text-sm font-medium">No npm packages found for &ldquo;{query}&rdquo;</p>
